...
// Same origin the built userscript already talks to (userscript/src/config.ts).
const PRODUCTION_ORIGIN = 'https://readsync-n7zp.onrender.com';
...
export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, path.resolve(__dirname, '..'), [
    'API_KEY',
    'USERSCRIPT_UPDATE_TOKEN',
  ]);
  const apiKey = (process.env.API_KEY ?? env.API_KEY ?? '').trim();
  if (!apiKey || apiKey.length > 256 || /[\r\n]/.test(apiKey)) {
    throw new Error('Set API_KEY in the project .env or build environment before building the userscript.');
  }
  const updateToken = (
    process.env.USERSCRIPT_UPDATE_TOKEN ?? env.USERSCRIPT_UPDATE_TOKEN ?? ''
  ).trim();
  if (!updateToken || /[\r\n/?#]/.test(updateToken)) {
    throw new Error('Set USERSCRIPT_UPDATE_TOKEN in the project .env or build environment before building the userscript.');
  }
  const USERSCRIPT_URL = `${PRODUCTION_ORIGIN}/u/${updateToken}/readsync.user.js`;
  return {
  define: { __READSYNC_API_KEY__: JSON.stringify(apiKey) },
  plugins: [injectUserscriptHeader(buildUserscriptHeader(USERSCRIPT_URL))],
  build: {
    lib: {
      entry: 'src/main.ts',
